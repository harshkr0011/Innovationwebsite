...
import { User, Award, BookOpen, Briefcase, Share2, Edit3, Eye, Settings, Github, Linkedin, Globe, Save, History, Link as LinkIcon, Camera } from 'lucide-react';
...
const Profile = () => {
    ...
    const defaultUser = {
        ...
        coverImage: "linear-gradient(45deg, #3b82f6, #8b5cf6)",
    ...
    const fetchProfile = async () => {
        try {
            ...
            const mappedUser = {
                ...
                bio: profileData.bio || "",
                avatar: profileData.user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${profileData.user?.username || 'User'}`,
                coverImage: profileData.user?.coverImage || "linear-gradient(45deg, #3b82f6, #8b5cf6)",
                stats: {
    ...
    return (
        <div className="container" style={{ maxWidth: '1000px', margin: '0 auto', padding: '2rem' }}>
                ...
                <div className="glass-panel" style={{ padding: '0.5rem', display: 'flex', gap: '0.5rem', borderRadius: '999px' }}>
                    <button
                        ...
                        style={{
                            ...
                            color: viewMode === 'owner' ? 'white' : 'var(--text-muted)',
                            border: 'none',
                            cursor: 'pointer'
                        }}>
                        Edit View
                    ...
                    <button
                        ...
                        style={{
                            ...
                            color: viewMode === 'recruiter' ? 'white' : 'var(--text-muted)',
                            border: 'none',
                            cursor: 'pointer',
                            display: 'flex',
                            alignItems: 'center',
            ...
            <div className="glass-panel" style={{ position: 'relative', overflow: 'hidden', padding: 0 }}>
                ...
                <div style={{
                    height: '200px',
                    background: user.coverImage?.startsWith('data:') || user.coverImage?.startsWith('http') ? `url(${user.coverImage}) center/cover no-repeat` : user.coverImage
                }}></div>

                <div style={{ padding: '0 3rem 3rem', flexDirection: 'column', display: 'flex' }}>
                    <div className="flex-stack-mobile" style={{ justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '-60px', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
                        <div className="flex-stack-mobile" style={{ alignItems: 'flex-end', gap: '2rem', flex: '1', minWidth: '300px' }}>
                            <img src={user.avatar} alt="Profile" style={{ width: '150px', height: '150px', borderRadius: '50%', border: '4px solid var(--bg-card)', background: 'var(--bg-card)', objectFit: 'cover', flexShrink: 0 }} />
                            <div style={{ marginBottom: '1rem', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', padding: '0.5rem 1rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)' }}>
                                <h1 style={{ fontSize: '2.5rem', marginBottom: '0.25rem', lineHeight: 1, color: 'white' }}>{user.name}</h1>
                                <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '1.1rem', margin: 0 }}>{user.role}</p>
                            </div>
                        </div>
                        <div style={{ marginBottom: '2rem', display: 'flex', gap: '1rem', alignSelf: 'flex-end' }}>
                            {viewMode === 'owner' ? (
                                <button
                        ...
                        <div style={{ display: 'grid', gap: '1rem' }}>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1rem' }}>
                                <div>
                                    ...
                                    <div style={{ position: 'relative', height: '100px', borderRadius: '8px', overflow: 'hidden', background: user.coverImage?.startsWith('data:') || user.coverImage?.startsWith('http') ? `url(${user.coverImage}) center/cover no-repeat` : user.coverImage }}>
                                        <button onClick={() => document.getElementById('modal-cover-upload').click()} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)', color: 'white', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', opacity: 0.8, transition: 'opacity 0.2s' }}>
                                            <Camera size={20} /> Change Cover
                                        </button>
                                        <input type="file" id="modal-cover-upload" hidden accept="image/*" onChange={(e) => handleImageChange(e, 'coverImage')} />
                                    </div>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                    <img src={user.avatar} alt="Avatar" style={{ width: '64px', height: '64px', borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--glass-border)' }} />
                                    <button onClick={() => document.getElementById('modal-avatar-upload').click()} className="btn-secondary" style={{ padding: '0.5rem 1rem', display: 'flex', gap: '0.5rem', alignItems: 'center', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--glass-border)', borderRadius: '6px', color: 'var(--text-main)', cursor: 'pointer' }}>
                                        <Camera size={16} /> Change Profile Picture
                                    </button>
                                    <input type="file" id="modal-avatar-upload" hidden accept="image/*" onChange={(e) => handleImageChange(e, 'avatar')} />
                                </div>
                            </div>

                            <div>
                                <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Name</label>
                                <input name="name" value={user.name} onChange={handleEditChange} className="glass-input" style={{ width: '100%' }} />
                            </div>
                            <div>
