...
router.post('/chat', async (req, res) => {
    try {
        ...
        const getMockChat = (msg) => {
            const responses = [
                "That's an interesting angle. Have you considered how this scales to 10,000 users?",
                "Based on what you're building, I'd suggest focusing on the MVP features first.",
                "I can see this working well. Don't forget to validate your assumptions with real users.",
                "Great progress! Maybe look into potential partnerships for this stage.",
                "From a technical standpoint, ensure your architecture handles real-time data efficiently."
            ];
            return {
                reply: responses[Math.floor(Math.random() * responses.length)],
                suggestion: "Would you like to generate a tasks list for this?"
            };
        };

        if (!model) {
            console.log("Serving Smart Mock for Chat (Gemini unavailable)");
            await new Promise(resolve => setTimeout(resolve, 600));
            return res.json(getMockChat(message));
        }

        const prompt = `You are an experienced, encouraging, and strategic Startup Co-Founder AI. 
        User is currently on page: "${context?.page || 'Unknown'}".
        User Role: "${context?.role || 'Founder'}".
        
        User says: "${message}"
        
        Provide a helpful, concise response (max 2-3 sentences). 
        Then offer one short, actionable strategic suggestion.
        Return JSON: { "reply": string, "suggestion": string }`;
        ...
        try {
            ...
            return res.json({ reply: text, suggestion: "Keep building!" });
        }

    } catch (err) {
        console.error("AI Chat Error:", err.message);
        if (!res.headersSent) {
            res.json({ reply: "I'm having trouble connecting right now, but keep pushing forward!", suggestion: "Try refreshing the page." });
        }
    }
});
