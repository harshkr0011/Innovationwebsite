...
// @route   PUT api/projects/:id/workspace/kanban/move
// @desc    Move task between columns
// @access  Private
router.put('/:id/workspace/kanban/move', auth, async (req, res) => {
    try {
        const { taskId, targetColumn } = req.body;
        const project = await Project.findById(req.params.id);
        ...
        // Find current column and task index
        let sourceColumn = '';
        let taskIndex = -1;

        // Find the task in one of the columns
        for (const col of ['todo', 'inProgress', 'done']) {
            const index = project.workspace.kanban[col].findIndex(t => t._id.toString() === taskId);
            if (index !== -1) {
                sourceColumn = col;
                taskIndex = index;
                break;
        ...
        // If trying to move to same column, just return
        if (sourceColumn === targetColumn) {
            return res.json(project.workspace.kanban);
        }

        // Get the task
        const task = project.workspace.kanban[sourceColumn][taskIndex];

        // Remove from source
        project.workspace.kanban[sourceColumn].splice(taskIndex, 1);

        // Update task timestamps based on column
        ...
        // Add to target
        project.workspace.kanban[targetColumn].push(task);
        ...
        project.contributions.push({
            user: req.user.id,
            action: 'moved_task',
            description: `Moved task "${task.title}" to ${targetColumn}`,
            timestamp: new Date()
        });
